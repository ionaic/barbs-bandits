...
class TextElement : public Element {
    public:
        ...
        TextElement(unsigned int x, unsigned int y) { TextElement(x, y, 0, 0 ,0, ""); }
        //CREATING A TEXT ELEMENT WITH 0 font size and no text causes a seg fault in text (below)
        ...
        TextElement(unsigned int x, unsigned int y, unsigned int width, unsigned int height, int size, string txt) : Element(x, y, width, height) {
        ...
        void clearResult() {
        	...
        	if (_temp) _temp->blit(*(this->_result), 0U, 0U, 0U, 0U, this->_width, this->_height);
        	else
        		cout << "ERROR" << endl;
        }
